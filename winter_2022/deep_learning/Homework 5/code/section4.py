...
def propogation(change, X_train, Y_train, X_test, Y_test, epoch, loss_v_epoch_train, loss_v_epoch_test, num_epoch, eta, layers):
    while epoch < 5000:
...
def MLP():
    ...
    FC1, RL, FC2, SIG, SQ = classes1(X_train)
    layers = [IL, FC1, RL, FC2, SIG, SQ]
    eta = .0001
    epoch = 0
    change = 0
    loss_v_epoch_train = []
    ...
    propogation(change, X_train, Y_train, X_test, Y_test, epoch, loss_v_epoch_train, loss_v_epoch_test, num_epoch, eta, layers)
    
    FC1, LIN, FC2, TAN, SQ = classes2(X_train)
    layers = [IL, FC1, LIN, FC2, TAN, SQ]
    eta = .0001
    epoch = 0
    change = 0
    loss_v_epoch_train = []
    ...
    FC1, FC2, FC3, FC4, SM, CE = classes3(X_train)
    layers = [IL, FC1, FC2, FC3, FC4, SM, CE]
    eta = .0001
    epoch = 0
    change = 0
    loss_v_epoch_train = []
...
def classes3(X):
    ...
    d_out4 = 10
    FC1 = FullyConnectedLayer(d_in, d_out1)
    FC2 = FullyConnectedLayer(d_out1, d_out2)
    FC3 = FullyConnectedLayer(d_out2, d_out3)
    FC4 = FullyConnectedLayer(d_out3, d_out4)
    SM = SoftmaxLayer()
    CE = CrossEntropy()
    return FC1, FC2, FC3, FC4, SM, CE