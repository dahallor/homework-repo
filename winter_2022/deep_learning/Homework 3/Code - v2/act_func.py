...
class SigmoidLayer(Layer):
    ...
    def gradient(self):
        ...
        k = len(Y[0]) 
        n = len(Y)
        delta = numpy.zeros((n, 16, 16), dtype = float, order = 'C')
        #print(delta)


        for a in range(n):
            for i in range(4):
                delta[a][i][0] = Y[a][0] * (1 - Y[a][0]) + epsilon
                delta[a][i][5] = Y[a][0] * (1 - Y[a][0]) + epsilon
                delta[a][i][10] = Y[a][0] * (1 - Y[a][0]) + epsilon
                delta[a][i][15] = Y[a][0] * (1 - Y[a][0]) + epsilon
